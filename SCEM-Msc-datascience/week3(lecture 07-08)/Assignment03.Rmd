...
S = sum((x-mean(X))*(y-mean(Y)))/n

S*n = sum((x-mean(X))*(y-mean(Y))= sum(x*y-x*mean(Y)-y*mean(X)+mean(X)*mean(Y))
x_new = x*a +b
y_new = y*c +d


S_new = sum((x_new-mean(X))*(y_new-mean(Y)))/n 
S_new*n = sum((x*a+b-a*mean(X))*(y*c+d-c*mean(Y)))
        = a*c*sum((x+b-mean(X))*(y+d-mean(Y)))
        = a*c*sum(x*y+x*d-x*mean(Y)+b*y+b*d-b*mean(Y)-y*mean(X)+d*mean(X)+mean(X)*mean(Y))
        = a*c*sum(x*y-x*mean(Y)-y*mean(X)+mean(X)*mean(Y))) + a*c*sum(x*d+b*y+b*d-b*mean(Y)+d*mean(X))
        = S*n+  a*c*sum(x*d+b*y+b*d-b*mean(Y)+d*mean(X))
S_new = S + a*c*sum(x*d+b*y+b*d-b*mean(Y)+d*mean(X))/n
...
S_new<- function(X,Y){
  ...
  n<-length(X)
  X_mean <- mean(Hawks[['Weight']],na.rm = TRUE)
  Y_mean <- mean(Hawks[['Wing']],na.rm = TRUE)
  S_new <- S + -2.4*sum(x*3+7.1*y+ 7.1*3-7.1*Y_mean+3*X_mean)/n
  return(S_new)
}
