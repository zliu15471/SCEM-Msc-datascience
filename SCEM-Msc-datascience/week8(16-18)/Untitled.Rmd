...
sampling_simulation<-data.frame(trial=seq(num_trials)) %>%
  mutate(sample_size = map(.x=trial, ~sample(c(5,100),5, replace = TRUE)))



#mle_x<-sum((sampling_simulation-mean(sampling_simulation))^2) / (sample_size)
#v<-x<-sum((sampling_simulation-mean(sampling_simulation))^2) / (sample_size-1)
