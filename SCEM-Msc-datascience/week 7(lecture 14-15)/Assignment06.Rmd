...
```{r}
samplemean<-beta%>%
  map(~ mean(sample_X_0310_new(beta)))

```
```{r}
samplemean<-data.frame(samplemean)
...
Expectation<-beta%>%
  map(~ mean(3*0.1 + beta*10))
Expectation<-c(Expectation)

Expectation<-data.frame(Expectation)
Expectation<-t(Expectation)
```

```{r}
data_frame_X<- data.frame(beta,Expectation,samplemean)
...
ggplot(data_frame_X,aes(beta,samplemean)) + geom_line()
```
...
inbuilt_cdf_output <- map_dbl(.x=test_inputs,.f=~pexp(q=.x,rate=lambda)) all.equal(my_cdf_output,inbuilt_cdf_output)
```
...
inbuilt_cdf_output <- map_dbl(.x=test_inputs,.f=~qexp(q=.x,rate=lambda)) all.equal(my_cdf_output,inbuilt_cdf_output)
