...
sampling_without_replacement_simulation<-sampling_without_replacement_simulation%>%
  ...
  mutate(greens = map_dbl(.x=sample_collections,~sum(.x>80 & .x<=100)))
...
5. 
```{r}
missing_values<-pmin(sampling_without_replacement_simulation$reds,sampling_without_replacement_simulation$blues,sampling_without_replacement_simulation$greens)
```
6.
```{r}
missing_values%>%
  table(missing_values==0)
portion<- 125/1000
print(portion)
```
